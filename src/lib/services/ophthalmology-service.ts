...
export const ophthalmologyService = {
  ...
  async getExams(filters?: { status?: string; clinic_id?: string }): Promise<OphthalmologyExam[]> {
    let query = supabase.from("ophthalmology_exams").select("*").order("created_at", { ascending: false });
  ...
  async getExamById(id: string): Promise<OphthalmologyExam> {
    const { data, error } = await supabase.from("ophthalmology_exams").select("*").eq("id", id).single();
    if (error) throw new Error(error.message);
    return data;
  },

  async createExam(exam: OphthalmologyExamInsert): Promise<OphthalmologyExam> {
    const { data, error } = await supabase.from("ophthalmology_exams").insert(exam).select().single();
    if (error) throw new Error(error.message);
    return data;
  },

  async updateExam(id: string, updates: OphthalmologyExamUpdate): Promise<OphthalmologyExam> {
    const { data, error } = await supabase.from("ophthalmology_exams").update(updates).eq("id", id).select().single();
    if (error) throw new Error(error.message);
    return data;
  },
  ...
  async getPrescriptionsByExam(examId: string): Promise<OphthalmologyPrescription[]> {
    const { data, error } = await supabase.from("ophthalmology_prescriptions").select("*").eq("exam_id", examId).order("created_at", { ascending: false });
    if (error) throw new Error(error.message);
    return data ?? [];
  },

  async getPrescriptionsByDoctor(doctorId: string): Promise<OphthalmologyPrescription[]> {
    const { data, error } = await supabase.from("ophthalmology_prescriptions").select("*").eq("doctor_id", doctorId).order("created_at", { ascending: false });
    if (error) throw new Error(error.message);
    return data ?? [];
  },

  async createPrescription(prescription: OphthalmologyPrescriptionInsert): Promise<OphthalmologyPrescription> {
    const { data, error } = await supabase.from("ophthalmology_prescriptions").insert(prescription).select().single();
    if (error) throw new Error(error.message);
    return data;
  },
